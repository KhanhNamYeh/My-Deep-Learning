...
class VideoProcessor():
    ...
    def video_process(self, frame_process: FrameProcessor) -> None:
        ...
        while True:
            ...
            # make train_data
            # Save frame image
            cv2.imwrite(f"train_data/images/frame_{frame_idx:04d}.png", frame)
    ...
    def labels(input_path: str, output_path: str, width: int, height: int) -> None:
        ...
        os.makedirs("train_data/boundings", exist_ok=True)
        for i in range(3087):
            with open(f"train_data/boundings/frame_{i:04d}.txt", "w") as f:
                pass
...
def main(argv):   
    ...
    if len(argv) == 1 or argv[1] == "transform":
        video_processor.labels("label/true_label", "train_data/boundings", info['size'][0], info['size'][1])
